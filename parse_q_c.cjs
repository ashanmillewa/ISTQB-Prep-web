...
const qText = fs.readFileSync('qc.txt', 'utf8');
const aText = fs.readFileSync('ac.txt', 'utf8');

// Parse answers from Paper C
const answers = [];
...
for (let i = 0; i < qLines.length; i++) {
  ...
  if (qMatch && !line.includes('..................')) {
    if (currentQ) {
      ...
      questions.push({
        id: currentQ + 80, // offset for paper C
...
let cIndex = qTsCode.indexOf('"paper-c": [');
if (cIndex !== -1) {
    // Replace existing
    qTsCode = qTsCode.substring(0, cIndex) + qCBlock + "\n};\n";
} else {
    // Append
    qTsCode = qTsCode.replace(/};\s*$/, `,\n${qCBlock}\n};\n`);
}
...
let acIndex = aTsCode.indexOf('"paper-c": [');
if (acIndex !== -1) {
    // Replace existing
    aTsCode = aTsCode.substring(0, acIndex) + aCBlock + "\n};\n";
} else {
    // Append
    aTsCode = aTsCode.replace(/};\s*$/, `,\n${aCBlock}\n};\n`);
}
fs.writeFileSync(aPath, aTsCode);

console.log("Parsed Paper C successfully. Found " + outputQuestions.length + " questions.");
